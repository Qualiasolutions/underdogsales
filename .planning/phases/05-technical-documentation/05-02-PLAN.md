...
must_haves:
  truths:
    - "Developer can find documentation for any current API endpoint"
    - "Developer knows authentication requirements for each endpoint"
    - "Developer understands request/response schemas"
  artifacts:
    - path: "docs/API.md"
      provides: "All 12 current API endpoints with schemas, auth, errors (excludes future Retell endpoints from Phase 7)"
      min_lines: 300
  key_links:
    - from: "docs/API.md"
      to: "src/app/api/**/*.ts"
...
Output: Single API reference document covering all 12 current endpoints with request/response schemas.

Note: Retell endpoints (/api/retell/*) are part of Phase 7 (Voice Platform Migration) and will be documented when that phase is executed.
</objective>

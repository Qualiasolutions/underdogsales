...
must_haves:
  truths:
    ...
    - "Call can be ended and session saves correctly via savePracticeSession"
    - "handleCallEnd callback triggers savePracticeSession for Retell calls"
  ...
  key_links:
    ...
    - from: "src/components/voice/VoicePractice.tsx"
      to: "src/lib/actions/practice-session.ts"
      via: "onCallEnd -> saveTranscript -> savePracticeSession"
      pattern: "savePracticeSession"
---
...
7. **CRITICAL - Session Saving Wiring:** Verify the existing onCallEnd handler works for Retell:
   - The existing onCallEnd callback in startRoleplaySession triggers saveTranscript()
   - saveTranscript() calls savePracticeSession() which saves to database
   - Ensure the same callback pattern is used for startRetellSession:
     ```typescript
     onCallEnd: () => {
       // This triggers the existing saveTranscript flow
       // which calls savePracticeSession from practice-session.ts
       // No changes needed - callback is provider-agnostic
     }
     ```
   - The key is that onCallEnd callback from RetellSessionOptions matches the existing callback signature
...
IMPORTANT: Keep all existing VAPI logic intact. We're adding Retell as an alternative, not replacing VAPI.
  ...
  <verify>
    ...
    - Session saving intact: `grep -c "savePracticeSession" src/components/voice/VoicePractice.tsx` returns at least 1
    - onCallEnd used for both: `grep -A5 "onCallEnd" src/components/voice/VoicePractice.tsx` shows callback with transcript saving
  ...
  <done>
    ...
    - Session saves correctly after Retell call ends (via existing onCallEnd -> savePracticeSession flow)
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    3. Test with VAPI (default):
       ...
       - End call and verify it redirects to results page (session saved)
    ...
    5. Test with Retell (if agents configured in Plan 07-01):
       - Add `NEXT_PUBLIC_VOICE_PROVIDER=retell` to .env.local
       - Add `RETELL_API_KEY=your_key` to .env.local
       - Verify persona retellAgentIds are set in src/config/personas.ts
       - Restart dev server
       - Start a practice call with Retell
       - Speak a few sentences and end call
       - **CRITICAL**: Verify redirect to /practice/results/{sessionId}
       - This confirms session saved correctly via savePracticeSession

    6. Verify session in database:
       - Check Supabase dashboard -> roleplay_sessions table
       - Confirm new session exists with transcript data
  </how-to-verify>
  <resume-signal>Type "approved" if build passes, VAPI works, and (if Retell tested) session saves correctly after Retell call, or describe issues</resume-signal>
</task>

  ...
  - service: retell
    ...
    dashboard_config:
      - task: "Set webhook URL to {SITE_URL}/api/retell/webhook"
        location: "Retell Dashboard -> Agent Settings -> Webhook (after agents are created)"

must_haves:
  truths:
    - "6 Retell agents exist matching VAPI personas"
...
<task type="auto">
  <name>Task 0: Create Retell LLM + Agent pairs for all 6 personas</name>
  <files>
    src/config/personas.ts (update with agent IDs after creation)
  </files>
  <action>
Create 6 Retell LLM response engines and agents using Retell MCP tools (mcp__zapier__retell_ai_*).

For each persona in src/config/personas.ts (skeptical_cfo, busy_vp_sales, friendly_gatekeeper, defensive_manager, interested_but_stuck, aggressive_closer):

1. **Create Retell LLM** using mcp__zapier__retell_ai_create_retell_llm:
   - `general_prompt`: Use the full persona prompt from PERSONA_PROMPTS[personaId]
   - `begin_message`: Short greeting matching persona (e.g., "Yes?" for CFO)
   - `model`: "gpt-4.1"
   - `model_temperature`: 0.7
   - Note the returned `llm_id`

2. **Create Retell Agent** using mcp__zapier__retell_ai_create_a_phone_number OR via direct agent creation:
   - Actually, use the Retell SDK approach since MCP tools are limited
   - Alternative: Document the agent IDs after manual creation in dashboard

**Recommended approach**: Since MCP tools for Retell don't include direct agent creation with LLM attachment:
- Create agents via Retell Dashboard OR
- Create a temporary script using retell-sdk after Task 1 installs the SDK

**Interim solution for Task 0:**
- List existing agents: `mcp__zapier__retell_ai_list_agents` to check what exists
- If agents don't exist, create a checkpoint for human to create them in dashboard with these specs:
  - Agent per persona with matching name (e.g., "Sarah Chen (CFO)")
  - LLM with persona prompt from PERSONA_PROMPTS
  - Voice IDs: Use ElevenLabs voice IDs from personas.ts
  - Enable end_call tool
  </action>
  <verify>
    - `mcp__zapier__retell_ai_list_agents` returns 6 agents with names matching personas
    - Each agent has correct voice configuration
  </verify>
  <done>
    - 6 Retell agents exist (one per persona)
    - Agent IDs are documented for personas.ts update in Task 1
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
    Retell agents checked/created for all 6 personas.
  </what-built>
  <how-to-verify>
    1. Check existing agents via Retell MCP:
       - List agents to verify 6 exist with correct persona names

    2. If agents missing, create in Retell Dashboard (https://beta.retellai.com):
       - For each persona (Sarah Chen CFO, Marcus Johnson VP Sales, Emily Torres EA, David Park Manager, Lisa Martinez Ops, Tony Ricci Director):
         a. Create new Retell LLM with persona prompt from PERSONA_PROMPTS
         b. Create agent with:
            - Name: "{persona.name} ({persona.role})"
            - Response Engine: The LLM just created
            - Voice: ElevenLabs with voice ID from personas.ts
            - Enable "end_call" tool
            - Set webhook URL: {SITE_URL}/api/retell/webhook

    3. Record the 6 agent IDs for updating personas.ts in next task
  </how-to-verify>
  <resume-signal>Provide the 6 Retell agent IDs in format: {persona_id: agent_id, ...}</resume-signal>
</task>
...
3. Update personas in src/config/personas.ts:
   - Add retellAgentId field to each persona using IDs from checkpoint above
   - Use the agent IDs provided by user in the checkpoint response
   - Example: `retellAgentId: 'agent_xxxxxxxxxxxx', // Retell agent ID`
   - If no IDs provided yet, use placeholder: `retellAgentId: '', // TODO: Add Retell agent ID`
  </action>
  <verify>
